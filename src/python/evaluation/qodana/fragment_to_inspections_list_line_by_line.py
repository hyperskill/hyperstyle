...
from src.python.evaluation.qodana.util.models import QodanaColumnName as QC, QodanaIssue
from src.python.evaluation.qodana.util.util import (
    get_inspections_dict, replace_inspections_on_its_ids, configure_model_converter_arguments
)
...
def __replace_inspections_to_its_ids_in_row(row: pd.Series, inspections_dict: Dict[str, int],
    ...
    issues_list = QodanaIssue.parse_list_issues_from_json(row_df.iloc[0][QC.INSPECTIONS.value])
    ...
    for index, fragment_line in enumerate(fragment_lines):
        ...
        fragment_df.iloc[index][QC.INSPECTIONS.value] = replace_inspections_on_its_ids(issues, inspections_dict,
