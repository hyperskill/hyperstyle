...
def test_file_with_issues(file_name: str, n_issues: int) -> None:
    ...
    with use_file_metadata(path_to_file) as file_metadata:
        ...
        issues = list(
            filter(lambda i: i.type != IssueType.INFO, filter_low_measure_issues(issues, Language.GO))
        )
...
FILE_WITH_ISSUE_INFO_TEST_DATA = [
    ...
    (
        ...
        IssuesTestInfo(
            n_cc=1, n_maintainability=1, n_func_len=1, n_code_style=1, n_best_practices=1, n_error_prone=1
        ),
    ),
    (
        "case10_gocritic_issues.go",
        IssuesTestInfo(n_cc=1, n_maintainability=1, n_func_len=1, n_best_practices=3),
    ),
    (
    ...
    (
        ...
        IssuesTestInfo(n_cc=1, n_maintainability=1, n_func_len=1, n_error_prone=3),
    ),
    ...
    (
        "case18_generics.go",
        IssuesTestInfo(n_cc=2, n_maintainability=2, n_func_len=2, n_code_style=1, n_info=4),
    ),
]
