...
def test_file_with_issues(file_name: str, n_issues: int) -> None:
    ...
    with use_file_metadata(path_to_file) as file_metadata:
        ...
        issues = list(
            filter(lambda i: i.type != IssueType.INFO, filter_low_measure_issues(issues, Language.PYTHON))
        )
...
FILE_NAMES_AND_N_ISSUES_INFO = [
    ...
    (
        ...
        IssuesTestInfo(n_code_style=1, n_best_practices=4, n_error_prone=1, n_cc=8, n_other_complexity=1),
    ),
    ...
    ("case6_unused_variables.py", IssuesTestInfo(n_best_practices=3, n_cc=1)),
    ("case8_good_class.py", IssuesTestInfo(n_cc=1, n_cohesion=1)),
    ("case7_empty_lines.py", IssuesTestInfo(n_cc=5, n_cohesion=2)),
    ("case10_unused_variable_in_loop.py", IssuesTestInfo(n_best_practices=1, n_cc=1)),
    ...
    ("case14_returns_errors.py", IssuesTestInfo(n_best_practices=1, n_error_prone=3, n_cc=4)),
    ("case35_line_break.py", IssuesTestInfo(n_best_practices=1, n_code_style=10, n_cc=1)),
